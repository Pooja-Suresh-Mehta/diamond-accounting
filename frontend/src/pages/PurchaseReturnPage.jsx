...
export default function PurchaseReturnPage() {
  ...
  const [opts, setOpts] = useState({ types: ['LOCAL', 'External', 'Internal', 'Pend Sale', 'HOLD', 'LAB'], sub_types: ['Bank', 'Cash'], categories: [], currencies: ['USD', 'INR', 'AED'], currency_rates: {}, parties: [], brokers: [], payment_statuses: [], lot_numbers: [], lot_items: [], next_memo_number: '1' });

  const loadRows = async () => {
    const res = await api.get('/parcel/purchase-return', { params: { search } });
    setRows(Array.isArray(res.data) ? res.data : []);
    setPage(1);
  };
  const loadOpts = async () => {
    const res = await api.get('/parcel/purchase-return/options');
    setOpts(res.data);
  };
  const loadEdit = async () => {
    if (!id) return;
    const res = await api.get(`/parcel/purchase-return/${id}`);
  ...
  useEffect(() => { if (!isFormMode) loadRows().catch(() => toast.error('Failed to load purchase returns')); }, [search, isFormMode]);
  useEffect(() => {
    if (isEditMode) loadEdit().catch(() => toast.error('Failed to load purchase return'));
    if (isAddMode) {
      setForm({ ...INIT, memo_number: String(opts.next_memo_number || '1') });
      setLotDraft({ ...INIT_ITEM });
    }
  }, [isEditMode, isAddMode, id, opts.next_memo_number]);
  ...
  const exportExcel = () => {
    ...
    rows.forEach(r => {
      csvRows.push([r.memo_number,r.inv_bill_no,r.date,r.print_date||'',r.party,r.purchase_type,r.sub_type,r.category,Number(r.total_carats||0).toFixed(2),Number(r.total_amount||0).toFixed(2),r.currency,Number(r.inr_amt||0).toFixed(2),Number(r.usd_amt||0).toFixed(2),r.due_date||'',r.payment_status].join(','));
    });
  ...
  const save = async () => {
    if (!form.memo_number.trim()) return toast.error('Memo Number is required');
  ...
  const removeRow = async (rid) => {
    if (!confirm('Delete purchase return?')) return;
    await api.delete(`/parcel/purchase-return/${rid}`);
    await loadRows();
  };
          ...
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  <th className="text-left px-3 py-2">Action</th><th className="text-left px-3 py-2">Invoice</th><th className="text-left px-3 py-2">BillNo</th>
                  <th className="text-left px-3 py-2">Date</th><th className="text-left px-3 py-2">Print Date</th><th className="text-left px-3 py-2">Party</th>
                  ...
                  <th className="text-right px-3 py-2">Carats</th><th className="text-right px-3 py-2">Amount</th>
                  ...
                  <th className="text-left px-3 py-2">DueDate</th><th className="text-left px-3 py-2">Payment Status</th>
                </tr>
              </thead>
              <tbody>
                {pagedRows.map((r) => (
                  <tr key={r.id} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right">{Number(r.total_carats || 0).toFixed(2)}</td><td className="px-3 py-2 text-right">{Number(r.total_amount || 0).toFixed(2)}</td>
                    <td className="px-3 py-2">{r.currency}</td><td className="px-3 py-2 text-right">{Number(r.inr_amt || 0).toFixed(2)}</td>
                    <td className="px-3 py-2 text-right">{Number(r.usd_amt || 0).toFixed(2)}</td><td className="px-3 py-2">{r.due_date || ''}</td><td className="px-3 py-2">{r.payment_status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Purchase Return / Add</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <F label="Memo Number" name="memo_number" value={form.memo_number} onChange={setValue} readOnly />
          <F label="Inv/Bill No" name="inv_bill_no" value={form.inv_bill_no} onChange={setValue} />
          ...
          <F label="Broker" name="broker" value={form.broker} onChange={setValue} options={opts.brokers} searchable />
        ...
        <div className="border-t pt-5 grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2 items-center">
              <span className="font-semibold text-gray-700">CGST%</span><input type="number" className="px-3 py-2 text-sm border rounded" value={form.cgst_pct} onChange={(e) => setValue('cgst_pct', e.target.value)} /><input type="number" className="px-3 py-2 text-sm border rounded bg-gray-100 text-right" value={form.cgst_amount} readOnly />
            </div>
            <div className="grid grid-cols-3 gap-2 items-center">
              <span className="font-semibold text-gray-700">SGST%</span><input type="number" className="px-3 py-2 text-sm border rounded" value={form.sgst_pct} onChange={(e) => setValue('sgst_pct', e.target.value)} /><input type="number" className="px-3 py-2 text-sm border rounded bg-gray-100 text-right" value={form.sgst_amount} readOnly />
            </div>
            <div className="grid grid-cols-3 gap-2 items-center">
              <span className="font-semibold text-gray-700">IGST%</span><input type="number" className="px-3 py-2 text-sm border rounded" value={form.igst_pct} onChange={(e) => setValue('igst_pct', e.target.value)} /><input type="number" className="px-3 py-2 text-sm border rounded bg-gray-100 text-right" value={form.igst_amount} readOnly />
            </div>
            <div className="grid grid-cols-3 gap-2 items-center">
              <span className="font-semibold text-gray-700">VAT%</span><input type="number" className="px-3 py-2 text-sm border rounded" value={form.vat_pct} onChange={(e) => setValue('vat_pct', e.target.value)} /><input type="number" className="px-3 py-2 text-sm border rounded bg-gray-100 text-right" value={form.vat_amount} readOnly />
            </div>
          </div>
