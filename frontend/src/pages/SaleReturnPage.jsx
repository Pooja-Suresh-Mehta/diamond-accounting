...
export default function SaleReturnPage() {
  ...
  const loadRows = async () => {
    const res = await api.get('/sale-return', { params: { search } });
    setRows(Array.isArray(res.data) ? res.data : []);
    setPage(1);
  };
  const loadOpts = async () => {
    const res = await api.get('/sale-return/options');
    setOpts(res.data);
  };
  const loadEdit = async () => {
    ...
    setForm({
      ...
      items: (base.items || []).map((item) => normalizeLineItem(item, {
        currency: base.currency, inrRate: base.inr_rate, aedRate: base.usd_rate,
      })),
    });
  };
  ...
  useEffect(() => { if (!isFormMode) loadRows().catch(() => toast.error('Failed to load sale returns')); }, [search, isFormMode]);
  useEffect(() => {
    if (isEditMode) loadEdit().catch(() => toast.error('Failed to load sale return'));
    if (isAddMode) {
    ...
    setForm((p) => {
      ...
      if (name === 'currency' || name === 'inr_rate' || name === 'usd_rate') {
        next.items = next.items.map((item) => normalizeLineItem(item, {
          currency: next.currency, inrRate: next.inr_rate, aedRate: next.usd_rate,
        }));
  ...
  const setItemValue = (name, value) => {
    setForm((p) => {
      setLotDraft((draft) => {
        ...
        return normalizeLineItem(raw, { currency: p.currency, inrRate: p.inr_rate, aedRate: p.usd_rate, sourceField: name });
      });
      return p;
    });
  };
  ...
  const setLessSign = (field, sign) => {
    setForm((p) => {
      setLotDraft((draft) => normalizeLineItem({ ...draft, [field]: sign }, {
  ...
  const setLotFromMaster = (lotNo) => {
    if (!lotNo) {
      return setForm((p) => {
        setLotDraft(normalizeLineItem({ ...INIT_ITEM }, { currency: p.currency, inrRate: p.inr_rate, aedRate: p.usd_rate }));
        return p;
      });
    }
    ...
    setForm((p) => {
      ...
      setLotDraft(normalizeLineItem(line, { currency: p.currency, inrRate: p.inr_rate, aedRate: p.usd_rate }));
      return p;
    });
  };
  ...
  const addSubmittedLot = () => {
    ...
    setForm((p) => ({
      ...p,
      items: [...p.items, normalizeLineItem(lotDraft, { currency: p.currency, inrRate: p.inr_rate, aedRate: p.usd_rate })],
    }));
    setLotDraft({ ...INIT_ITEM });
  };
  ...
  const exportExcel = () => {
    const headers = ['Invoice','BillNo','Date','Party','Type','Sub Type','Category','Carats','Amount','Currency','INR Amt','USD Amt','Due Date','Payment Status'];
    const csvRows = [headers.join(',')];
    rows.forEach(r => {
      csvRows.push([r.invoice_number,r.bill_no,r.date,r.party,r.purchase_type,r.sub_type,r.category,Number(r.total_carats||0).toFixed(2),Number(r.total_amount||0).toFixed(2),r.currency,Number(r.inr_amt||0).toFixed(2),Number(r.usd_amt||0).toFixed(2),r.due_date||'',r.payment_status].join(','));
    });
    const blob = new Blob([csvRows.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'sale_returns.csv'; a.click();
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...form,
        items: activeItems.map(({ less1_sign, less2_sign, less3_sign, ...rest }) => ({
          ...rest, less1: Number(rest.less1 || 0), less2: Number(rest.less2 || 0), less3: Number(rest.less3 || 0),
        })),
      };
      if (isEditMode) await api.put(`/sale-return/${id}`, payload);
      else await api.post('/sale-return', payload);
      toast.success(isEditMode ? 'Updated' : 'Created');
      navigate('/parcel-transaction/sale-return', { replace: true });
    } catch (e) {
  ...
  const removeRow = async (rid) => {
    if (!confirm('Delete sale return?')) return;
    await api.delete(`/sale-return/${rid}`);
    await loadRows();
  };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">Parcel Transactions / Sale Return</h1>
          <div className="flex gap-2">
            ...
            <button onClick={() => { navigate('/parcel-transaction/sale-return/add'); }} className="px-3 py-2 text-sm bg-blue-600 text-white rounded-lg flex items-center gap-1.5"><Plus className="w-4 h-4" /> Create Sale Return</button>
          </div>
        </div>
          ...
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  {['Action', 'Invoice', 'BillNo', 'Date', 'Party', 'Type', 'Sub Type', 'Category', 'Carats', 'Amount', 'Currency', 'INR Amt', 'USD Amt', 'DueDate', 'Payment Status'].map((h) => (
                ...
                {pagedRows.map((r) => (
                  <tr key={r.id} className="border-t border-gray-100">
                    <td className="px-3 py-2">
                      <div className="flex gap-2">
                        <button onClick={() => navigate(`/parcel-transaction/sale-return/edit/${r.id}`)} className="text-blue-600">Edit</button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Sale Return / {isEditMode ? 'Edit' : 'Add'}</h1>
