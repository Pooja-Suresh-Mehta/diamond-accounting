              ...
              <tbody>
                {pagedRows.map((r) => (
                  <tr key={r.id} className="border-t border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right">{Number(r.usd_amt || 0).toFixed(2)}</td><td className="px-3 py-2">{r.due_date || ''}</td><td className="px-3 py-2">{r.payment_status}</td><td className="px-3 py-2">{r.created_at ? new Date(r.created_at).toLocaleString() : ''}</td><td className="px-3 py-2">{r.created_by_name || ''}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                <tbody>
                  {importedRows.map((r, idx) => (
                    <tr key={`${r.invoice_number}-${r.lot_number}-${idx}`} className="border-t border-gray-100">
                      ...
                      <td className="px-2 py-1 text-right">{Number(r.selected_carat || r.issue_carats || 0).toFixed(2)}</td>
