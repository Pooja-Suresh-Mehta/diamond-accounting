...
const numericFields = new Set(['inr_rate', 'usd_rate', 'received_dr', 'paid_cr', 'amount', 'inr_amt', 'usd_amt', 'aed_amt']);
...
export default function PaymentReceiptsPage() {
  ...
  const fetchList = async () => {
    setLoading(true);
    try {
      const r = await api.get('/payments', { params: { search, page: 1, page_size: 50 } });
      setRows(r.data);
  ...
  const handleLoadInvoices = (type) => {
    if (!form.party_account) return toast.error('Select Transaction Account first');
    toast(`Loading ${type} invoices for ${form.party_account}...`);
    // Future: api.get('/payments/pending-invoices', { params: { party: form.party_account, type } })
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { ...form, amount: form.received_dr || form.paid_cr };
      if (isEdit) {
      ...
      setView('list');
      navigate('/financial/payment-receipts');
    } catch (e) {
  ...
  const exportExcel = () => {
    const headers = ['Date', 'Type', 'Main Account', 'Transaction Account', 'Currency', 'Received Dr', 'Paid Cr', 'Description'];
    const csvRows = [headers.join(',')];
    rows.forEach(r => {
      csvRows.push([r.date, r.vtype, `"${r.main_account || ''}"`, `"${r.party_account || ''}"`, r.currency, (r.received_dr || 0).toFixed(2), (r.paid_cr || 0).toFixed(2), `"${(r.description || r.narration || '').replace(/"/g, '""')}"`].join(','));
    });
    const blob = new Blob([csvRows.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'payment_receipts.csv'; a.click();
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setForm({ ...INIT }); setView('form'); navigate('/financial/payment-receipts/add'); }} className="px-3 py-2 text-sm bg-blue-600 text-white rounded-lg flex items-center gap-1.5"><Plus className="w-4 h-4" /> Add Payment/Receipt</button>
          </div>
        </div>
          ...
          <div className="overflow-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>{['Date', 'Type', 'Main Account', 'Transaction Account', 'Currency', 'Received Dr', 'Paid Cr', 'Actions'].map(h => (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        <button onClick={() => { setView('list'); navigate('/financial/payment-receipts'); }} className="text-sm text-blue-600 hover:underline">← Back</button>
        <h2 className="text-lg font-semibold">Payment / {isEdit ? 'Edit' : 'Add'} Payment &amp; Receipts</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col justify-end gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => handleLoadInvoices('Invoice')} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">On Invoice</button>
              <button onClick={() => handleLoadInvoices('Adat')} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">On Adat</button>
              <button onClick={() => handleLoadInvoices('Brokerage')} className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">On Brokerage</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="bg-gray-50 border-b">
            <tr>{['Date', 'Invoice Number', 'Date', 'BillNo', 'Carats', 'Inv.Cur', 'USD Amount', 'Base Rate', 'INR Amount', 'Adj USD Amt', 'Pay INR Rate', 'Adj INR Amt', 'Rem USD Amt', 'Rem INR Amt', 'TrnType', 'Invoice Code'].map(h => (
              <th key={h} className="px-3 py-2 text-left font-medium text-gray-600 whitespace-nowrap">{h}</th>
            ))}</tr>
          </thead>
          <tbody>
            {invoiceRows.length === 0 ? (
              <tr><td colSpan={16} className="text-center py-6 text-gray-400">Click On Invoice / On Adat / On Brokerage to load pending items</td></tr>
            ) : invoiceRows.map((r, i) => (
              <tr key={i} className="border-t hover:bg-gray-50">
                ...
                <td className="px-3 py-2">{r.pay_inr_rate}</td>
